...
export async function POST(
  ...
  // Relisting is valid only on the ended → unsold branch (the unsold
  // status); ended alone may still resolve to a winner.
  if (auction.status !== 'unsold') {
    return NextResponse.json(
      { error: 'Uuesti müüki saab esitada ainult müümata oksjoni kohta' },
      { status: 409 },
    )
  }
  ...
  const notification = await repos.create({
    ...
    data: {
      ...
      event: 'auction.relist_requested',
      channel: 'in_app',
      title: 'Müüki esitamise taotlus on esitatud',
      body: `Oksjon "${auction.title}" on esitatud uuesti müüki taotlemiseks.`,
