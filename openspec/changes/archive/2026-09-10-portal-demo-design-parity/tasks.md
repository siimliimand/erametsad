...
- [x] 9.2 Update DESIGN.md: portal token section + rewritten Mockup deviations section (parity baseline + remaining functional deviations) <!-- agent: fullstack-engineer.fast, depends_on: [9.1], touches: [DESIGN.md] -->
- [x] 9.3 Visual comparison: screenshot each portal page (`/`, `/oksjon/:id` open + sealed, `/ajalugu`, `/login`, `/register`, `/select-profile`, `/update-password`, `/user/*`, `/lepingud`, signing flow) against its demo HTML and fix remaining deltas <!-- agent: fullstack-engineer.build, depends_on: [9.1], touches: [] -->
