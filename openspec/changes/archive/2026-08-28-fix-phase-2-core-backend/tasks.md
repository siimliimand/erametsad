...
- [x] 9.2 Verification: workspace `test`, `typecheck`, `lint` green; `seed:reset` against local docker-compose Postgres; API-level smoke of the five §1.1 flows (lead → CRM row, open bid with autobidder + anti-snipe, sealed ceremony, contract signing, archive/stats consistency) <!-- agent: fullstack-engineer.build, depends_on: [1.2, 2.3, 2.4, 3.2, 3.3, 3.5, 4.4, 5.4, 5.5, 6.1, 7.1, 7.2, 8.1, 8.2, 9.1], touches: [] -->
- [x] 9.3 Update `docs/tasks.md` Phase 2 checkboxes to match verified reality; record the [S]/[L] deferrals in writing next to the section <!-- agent: fullstack-engineer.fast, depends_on: [9.2], touches: [docs/tasks.md] -->
