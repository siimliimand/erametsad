...
`User`, `Profile` (private/company), `CompanyAccessRequest`, `AuctionRight`, `Auction` (with full field model: location, forest data, pricing, content, packages; `area_ha`/`volume_m3` as real columns), `Bid` (append-only), `Autobidder`, `AuctionSubscription`, `Contract`, `ContractTemplate` (one row per version, nullable stored source `source_content`/`source_format`), `Lead` (with `county_id`, nullable `user_id` and `auction_id` for portal-submitted leads), `Notification`, `NotificationTemplate` (versioned e-mail/SMS templates), `AuditEntry` (append-only, hash-chained via `prev_hash`/`hash`, with `reason`/`session_id`/`ip_hash`/`user_agent` context columns), `Settings` (singleton: fees, anti-snipe and alapakkumine defaults, feature flags, maintenance mode), `MaintenanceWindow` (planned closure windows), plus CMS content tables (`Article` (category, SEO title/description, specialist author), `Content`, `Page`, `PageBlock`, `PageVersion`, `FaqCategory`, `FaqItem` (active flag), `Testimonial` (draft/published status, rating), `PartnerService`, `LegalDocument`, `Redirect` (hit counter), `Specialist`, `Media` (alt text, focal point)), and geographic tables (`County`, `Parish`), `StatisticsSnapshot`. Service requests carry the status enum `new → routed → teostatud → suletud` and a nullable `user_id` that portal-authenticated submissions stamp (anonymous marketing rows keep it NULL). Publish readiness gates now also reject a draft whose `minBidCents` is not greater than 0, so portal-submitted placeholder drafts cannot reach `scheduled` without a price.
