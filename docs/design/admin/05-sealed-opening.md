...
7. **Tühistamine (abort/void)** — available until winner confirm: typed reason mandatory + superadmin; outcome `voided`: every sealed bid is rejected (append-only status change, amounts never revealed), bidders notified "Oksjon tühistati", no fee. Otsus (D-4): the voided lot follows the immutable status machine — ended → unsold — and never rewinds to draft; `draft` is a pre-publish state, so re-listing is a new lot via the existing "Avalda uuesti" duplicate flow. Post-confirm voiding happens in 08 (contract void), not here.
...
## Data & API
No REST endpoints: the ceremony screen talks to the server actions in `(admin)/_actions/auctions.ts`; every call is re-checked server-side against `sealed:read`/`sealed:operate`. `sealedCeremonyStateAction(auctionId)` returns the frozen read model (checklist, phase, post-reveal ranked bids with ceremony-scoped identity); `signSealedOpenerAction` / `signSealedApproverAction` bind the two distinct sessions (30-min signature TTL); `revealSealedBidsAction` decrypts once and replays idempotently; `confirmSealedCeremonyWinnerAction` takes the typed keyword + opener re-auth and the explicit decision (müük / müümata / varupakkumine); `markSealedUnsoldShortcutAction` covers the no-valid-bids case; `voidSealedBidsAction` implements block 7. Otsus (D-5): the earlier REST draft (`GET :id/sealed-state`, `POST :id/open-sealed`, `:id/confirm-winner`, `:id/void`, `:id/mark-unsold`) is retired in favour of these actions. Decryption is server-side; plaintext amounts exist only in the action response to the signed-in opener session; everything logged (`sealed.sign_opener`, `sealed.sign_approver`, `sealed.reveal`, `sealed.winner_confirm`, `sealed.mark_unsold`, `sealed.house_backup`, `sealed.void`).
